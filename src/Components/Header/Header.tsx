...
const Header = () => {
  ...
  return (
    <div>
     <div
        className={`${
          mobileMenuActive ? "flex bg-white" : "hidden"
        } ${scrollPosition && "bg-white"} z-10 fixed md:flex w-[100vw] h-[100vh] md:h-fit justify-between p-5`}
      >
          ...
          {navigationLinksData.map((navLink) => (
            <li>
              <a
                ...
                className={`${activeSection === navLink ? 'text-rose-500' : 'text-stone-400'}  font-bold py-1 px-0 decoration-0 text-lg block relative tracking-[2px] whitespace-nowrap`}
                href={`#${navLink}`}
                onClick={(e) => {
      ...
      {!mobileMenuActive && <div className="w-full h-[50px] bg-white z-20 fixed md:hidden">
